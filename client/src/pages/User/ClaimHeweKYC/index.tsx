...
const ClaimHeweKYCPage = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const response = await Claim.hewe({ token });
                if (response) {
                    toast.success(t(response.data.message) || t('Withdraw HEWE successful'));
    ...
    useEffect(() => {
        if (!loadingClaim) {
            const timer = setTimeout(() => {
                navigate('/user/assets', { replace: true });
            }, 3000);
                ...
                {loadingClaim ? (
                    ...
                    <div className="flex flex-col items-center gap-10">
                        ...
                        <p className="text-2xl font-bold">
                            {t('Withdraw HEWE successful')}
                        </p>
                        <p className="text-gray-600">{t('Redirecting to assets...')}</p>
...
export default ClaimHeweKYCPage;
