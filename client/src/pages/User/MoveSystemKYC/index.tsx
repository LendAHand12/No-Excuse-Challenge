...
const MoveSystemKYC = () => {
  ...
  let { token, user_id, status } = parsed;
  if (status !== 'success' || !token || !user_id) {
  ...
  useEffect(() => {
    (async () => {
      if (status === 'success' && token && user_id) {
        await MoveSystem.register({ user_id, token })
          .then((response) => {
      ...
      <div className="min-h-screen bg-white text-gray-900 flex flex-col justify-center items-center">
        {loadingClaim ? (
          ...
          <div className="flex flex-col items-center gap-10">
            ...
            <p className="text-2xl font-bold">{`Confirmed successfully`}</p>
          </div>
        )}
        <Link
          to="/user/profile"
